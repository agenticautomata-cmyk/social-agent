...
import { opportunitiesUiCopy } from '../lib/opportunities-ui';
import {
  getTerminology,
  ...
  return (
    <div className="space-y-16">
      <section>
        ...
        {overviewSubline && <p className="text-paper-soft mt-1 text-sm">{overviewSubline}</p>}
      </section>
      ...
      <section>
        <div className="section-mark mb-4">
          <span>{t.pages.overview.sourcesSection}</span>
        </div>
                ...
                return (
                  <tr key={c.id} className="border-t border-paper-edge align-top">
                    <td className="py-4 pr-4">
                      <Link href={`/campaigns/${c.id}`} className="link font-bold">
                        {c.name}
                      </Link>
                      <div className="text-xs text-paper-muted mt-1 italic">
                        └─ {c.description?.toLowerCase()}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <span
                        className={`bracket ${c.autonomyMode === 'auto' ? 'text-accent' : c.autonomyMode === 'hitl' ? 'text-signal-warn' : 'text-paper-muted'}`}
                      >
                        {c.autonomyMode}
                      </span>
                    </td>
      ...
      <section>
        <div className="section-mark mb-4">
          <span>// §3 state distribution</span>
        </div>
              ...
              .map((s) => {
                ...
                return (
                  <div key={s.state} className="flex items-baseline gap-4">
                    ...
                    <div
                      className={`flex-1 ${s.state === 'published' ? 'text-accent' : s.state === 'failed' ? 'text-signal-alert' : s.state === 'script_drafted' ? 'text-signal-warn' : 'text-paper-soft'} overflow-hidden whitespace-nowrap`}
                    >
                      {fill}
                    </div>
